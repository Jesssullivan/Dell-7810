...
let record
    ...
    = { date = "2026-04-25"
      ...
      , serialSeconds = Some 0.022323
      , parallelSeconds = Some 0.001807
      , speedup = Some 12.3536
      , delta = Some 1.86265e-09
      , contractCeiling = "C2"
      , result = ChapelProbe.ProbeResult.pass
      , evidence = [ "data/captures/honey/chapel-host-probe-generic-2026-04-25.txt" ]
      , notes = [ "Generic-lane same-evening Chapel host characterization captured with an already-present target Chapel store path; not a generic-versus-RT comparison." ]
      , followUp = [ "Capture the same probe on the RT lane with matched SMI/hwlat context." ]
      }
