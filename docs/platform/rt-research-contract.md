...
The important workflow symmetry is deliberate: runner lanes validate
reproducibility and tooling, while `honey` lanes capture machine-under-test
truth. The concrete workflow enrollment and cacheability policy is tracked in a
separate runner-boundary slice; this RT slice only records the host-evidence
contract and the local projection tools.
