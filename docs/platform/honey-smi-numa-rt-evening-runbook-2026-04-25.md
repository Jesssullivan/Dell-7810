...
```bash
just platform-kernel-status-remote target=${DELL_7810_TARGET}
...
```bash
just platform-kernel-status-remote target=${DELL_7810_TARGET}
just platform-kernel-schedule-next-rt-remote target=${DELL_7810_TARGET}
```
...
```bash
just platform-kernel-status-remote target=${DELL_7810_TARGET}
just platform-validate-kernel-baseline-remote-rt target=${DELL_7810_TARGET}
just platform-smi-hwlat-series-remote \
  target=${DELL_7810_TARGET} \
  tag=rt-2026-04-25 \
  samples=3 \
...
```bash
just platform-kernel-status-remote target=${DELL_7810_TARGET}
just platform-validate-kernel-baseline-remote target=${DELL_7810_TARGET}
```
