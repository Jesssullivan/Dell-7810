...
| Chapel / NUMA host results | `partial` | generic-lane `HostNumaProbe` result exists with raw capture, turnkey replay, and result note | RT-lane Chapel capture and decision on whether stronger NUMA claims need a different Chapel execution model |
...
| NUMA / Chapel result figure | `partial` | generic-lane host probe output and result note exist | RT-lane paired result or explicit scope decision |
...
3. one RT-lane `HostNumaProbe.chpl` result or an explicit decision that it is
   out of scope for the first writing packet
