...
- `IF-003`, `IF-004F` through `IF-005R`, `IF-006` through `IF-015`
- `GPU-002`, `GPU-006`, `GPU-007`
- `CBL-001` through `CBL-005`
...
4. Mark one or two likely cable-exit zones with masking tape, then capture `CBL-004` and `CBL-005`.
...
4. Capture `IF-015` so the latch coupon has a real panel-thickness assumption.
5. Capture `IF-013` and `IF-014` at the rear top corner and rear edge.
6. Record `PATH-002` by mocking the install/removal sweep against current desk or wall clearance.
...
1. Capture `IF-003`, `IF-004F`, `IF-004M`, `IF-004R`, `IF-005F`, `IF-005M`, `IF-005R`, `IF-006`, `IF-007`.
2. Capture any obvious step, bead, or offset for `IF-016`.
3. Test the hook-in motion concept with cardboard and record `PATH-001`.

Notes:

- Measure the lower rail front, middle, and rear as separate rows. Do not compress rail drift back into one averaged dimension.
...
Once the bench notes and photo refs are entered, run:
...
Only use `just measurements-session-01-apply-scad-write` after the dry-run
diff looks correct. That write path is intentionally additive-only: incomplete
rows stay untouched in `measured-params.scad` instead of being forced back to
zero, and it refuses to write if the measured rows are missing real photo refs
or notes.
