...
| --- | --- | --- |
| `tinyland-inc/linux-xr` README Dell troubleshooting | Dell now has `docs/platform/t7810-rt-boot-troubleshooting.md` for the durable host procedure. `linux-xr` PR [#29](https://github.com/tinyland-inc/linux-xr/pull/29) replaces the long README troubleshooting body with a Dell pointer. | Merge `#29` once CI is green; then `linux-xr` should retain only supplier-side requirements and historical kernel-debug context. |
