...
| --- | --- | --- |
| `tinyland-inc/linux-xr` README Dell troubleshooting | Still contains detailed T7810/SMI/RT boot-failure notes. The current live-host acceptance boundary now points back here, and Dell now has `docs/platform/t7810-rt-boot-troubleshooting.md` for the durable host procedure. | Keep only supplier-side requirements and historical kernel-debug context in `linux-xr`; replace the long README body there with a pointer back to the Dell-owned troubleshooting note. |
