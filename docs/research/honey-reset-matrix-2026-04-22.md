...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A | April 22, 2026, `00:39` to `00:41` on the prior boot | degraded runtime state followed by display probing in the bad state | no | absent from the useful display path | present but EDID failed | `Cannot find any crtc or sizes`, `No EDID found on connector: DP-2`, `ring sdma0 timeout`, `device lost from bus` | overlay network unhealthy, LAN TCP/22 still answered, SSH auth stalled after publickey offer | fail |
| B | April 22, 2026, fresh boot at `01:19 EDT` after manual hard reset | full manual hard reset | yes | `connected`, `enabled`, Dell EDID present, `1920x1080` modes present | `connected`, `256`-byte EDID present, `5088x2544` and `3840x1920` modes present | only benign display init markers on this path, including `Failed to setup vendor infoframe on connector HDMI-A-2: -22` and `fb0: amdgpudrmfb frame buffer device` | SSH and overlay network recovered; host stable enough for normal remote inspection | pass |
| C | April 22, 2026, pre-check at `01:50 EDT`, reboot completed into a new boot at `01:59 EDT`, overlay network usable again around `02:02 EDT` | controlled warm reboot from a known-good state with Dell HDMI and Beyond attached | no | `connected` and `enabled` after reboot | `connected` after reboot | same benign boot markers as Run B; no `No EDID found`, `sdma0 timeout`, or `device lost from bus` | direct LAN SSH returned by `02:01:59`; overlay path from the operator machine stayed down through `02:01:26` and then recovered through a relay around `02:02:24` to `02:02:28` | pass, network-delayed |
| D | April 22, 2026, pre-check at `02:06 EDT`, services quiesced at `02:06:45` to `02:07:40 EDT`, new boot at `02:10 EDT` | historical warm reboot row after stopping `rke2-server`, `docker.service`, and `docker.socket` | no | `connected` and `enabled` after reboot | `connected` after reboot | same benign boot markers as Runs B and C; no `No EDID found`, `sdma0 timeout`, or `device lost from bus` | SSH and LAN `:22` returned by `02:10:40`; overlay network reachability returned by `02:11:04`, still via relay | historical only, out of policy for future runs |
...
- remote return was slower than local host recovery:
  - overlay reachability from the operator machine stayed down through the watch
    window ending at `02:01:26 EDT`
  - direct LAN SSH to the `honey` management LAN address was already working by
    `02:01:59 EDT`
  - by `02:02:24` to `02:02:28 EDT`, overlay reachability had recovered,
    initially via relay rather than a direct path
...
- remote return timing on this row was:
  ...
  - overlay network reachability working again by `02:11:04 EDT`
  - the recovered overlay path was still via relay, not direct
